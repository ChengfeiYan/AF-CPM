...
#### MODIFICATION BEGIN
            out, out_dict = run_model(model, processed_feature_dict, tag, args.output_dir)
            for i in range(len(out_dict.keys())):
                zlist = out_dict[i]["zlist"]
                zlist_numpy = [z for z in zlist]
                zstack = np.stack(zlist_numpy)
                np.save(os.path.join(args.output_dir, f"{tag}_{i+1}_zstack.npy"), zstack)
                del out_dict[i]["zlist"], zlist, zlist_numpy, zstack
#### MODIFICATION END
